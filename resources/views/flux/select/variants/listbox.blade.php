...
>
    ...
    <div x-show="open" x-cloak @click.away="open = false"
        ...
        <div class="p-2 border-b dark:border-gray-700">
            <flux:input icon="magnifying-glass" x-model="search" placeholder="Pencarian..." wire:model.live="search" />
        </div>
        <ul class="max-h-48 overflow-y-auto">
            @foreach($options as $value => $label)
                <li @click="selected = { value: '{{ $label->id }}', label: '{{ $label->name }}' }; open = false; @this.set('{{ $name }}', '{{ $label->id }}')"
                    class="cursor-pointer py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-700"
